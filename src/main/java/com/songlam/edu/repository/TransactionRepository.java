...
public interface TransactionRepository extends JpaRepository<Transaction, String> {
    ...
    @Query("SELECT SUM(t.amount) " +
            "FROM Transaction t " +
            "WHERE t.dateOfRecorded BETWEEN :start AND :end " +
            "AND t.type = :type")
    ...
    @Query("SELECT s.subjectName, SUM(t.amount) as total " +
            "FROM Transaction t " +
            ...
            "ORDER BY total DESC")
    List<Object[]> sumAmountGroupBySubjectAndType(
            @Param("start") LocalDate start,
            @Param("end") LocalDate end,
            @Param("type") String type);

    @Query("SELECT c.className, SUM(t.amount) as total " +
            "FROM Transaction t " +
            "LEFT JOIN t.schoolClass c " +
            "WHERE t.dateOfRecorded BETWEEN :start AND :end " +
            "AND t.type = :type " +
            "GROUP BY c.className " +
            "ORDER BY total DESC")
    List<Object[]> sumAmountGroupByClassAndType(
            @Param("start") LocalDate start,
            @Param("end") LocalDate end,
            @Param("type") String type);

    @Query("SELECT t.dateOfRecorded, SUM(t.amount) " +
            "FROM Transaction t " +
            "WHERE t.dateOfRecorded BETWEEN :start AND :end " +
            "AND t.type = :type " +
            "GROUP BY t.dateOfRecorded " +
            "ORDER BY t.dateOfRecorded")
    List<Object[]> sumAmountGroupByDayAndType(
            @Param("start") LocalDate start,
            @Param("end") LocalDate end,
            @Param("type") String type);

    @Query("SELECT MONTH(t.dateOfRecorded), SUM(t.amount) " +
            "FROM Transaction t " +
            "WHERE t.dateOfRecorded BETWEEN :start AND :end " +
            "AND t.type = :type " +
            "GROUP BY MONTH(t.dateOfRecorded) " +
            "ORDER BY MONTH(t.dateOfRecorded)")
    List<Object[]> sumAmountGroupByMonthAndType(
            @Param("start") LocalDate start,
            @Param("end") LocalDate end,
            @Param("type") String type);
